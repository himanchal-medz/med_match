...
def wrapper(dist_name):
    ...
    df_distributor = pd.read_csv('SanyogEnterprises_29Dec_16156SKUs_data.csv', encoding='ISO-8859-1') # New Distributor data to be mapped
    ...
    df_master_syp = df_master.reset_index(drop = True) 
    #[df_master['medicine_type']=='others'].reset_index(drop = True)
    ...
    def preprocess_syrups():
        ...
        df_syp['mod_brand'] = (df_syp['mod_brand'].str.split()
                                  .apply(lambda x: OrderedDict.fromkeys(x).keys())
                                  .str.join(' '))
        df_master_syp.to_csv('error.csv')
